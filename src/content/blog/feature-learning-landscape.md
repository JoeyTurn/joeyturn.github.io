...
For reference $\mu$P \citet{yang:2021-tensor-programs-IV} tells you how you need to scale all of your network's (scale/learning-rate) hyperparameters in order for networks to stabily learn non-trivial functions even at infinite width, with this new perspective based on the ideas of network layers being 'balanced' (drawing back all the way to \citet{NEURIPS2018_fe131d7f}), with \citet{kunin2024richquickexactsolutions} describing how different amounts of balancedness affect feature learning. Note the balancedness picture's lacks of mentioning stability for large width networks: as long as you make two subsequent layers have 'a similar scale,' that's all you need!
