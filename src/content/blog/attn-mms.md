...
Secondly, we need to ensure these are proper probability maps, such that each row of $\mathbf{C}$ sums to 1. We'll do this by normalizing each row of $\mathbf{C}$ using $\mathbf{D} = \sum_{t=1}^{L-1} \vec{x}_t\vec{x}_{t}^\top$, with our final probability matrix being $\mathbf{T} = \mathbf{D}^{-1}\mathbf{C}$.
...
With the unfortunate news that linear attention fails at the ICLR task, then what is the simplest model that can perform the task?
